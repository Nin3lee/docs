...
该选项用以控制是否为响应式图像自动添加全局样式，除非你在手动为图像配置样式，否则我们建议你启用该选项。

该选项仅在通过配置项或图片组件的 `layout` 属性将布局模式设置为 `constrained`、`full-width` 或 `fixed` 时生效。

更多信息请参阅[图像指南](/zh-cn/guides/images/#响应式图像样式)。
...
响应式图像的默认布局类型。可以通过图像组件上的 `layout` 属性进行覆盖。

- `constrained` - 图像将缩放以适应容器，保持其纵横比，但不会超过指定的尺寸。
- `fixed` - 图像将保持其原始尺寸。
- `full-width` - 图像将缩放以适应容器，并保持其纵横比。

更多细节请参阅 [`layout` 组件属性](/zh-cn/reference/modules/astro-assets/#layout)。
...
响应式图像的默认 [`object-fit` CSS 属性值](https://developer.mozilla.org/zh-CN/docs/Web/CSS/object-fit)。可以通过图像组件上的 `fit` 属性进行覆盖。需设置 `layout` 的值。

更多细节请参阅 [`fit` 组件属性](/zh-cn/reference/modules/astro-assets/#fit)。
...
响应式图像的默认 [`object-position` CSS 属性值](https://developer.mozilla.org/zh-CN/docs/Web/CSS/object-position)。可以通过单个图像组件上的 `position` 属性进行覆盖。需设置 `layout` 的值。

更多细节请参阅 [`position` 组件属性](/zh-cn/reference/modules/astro-assets/#position)。
...
用于生成响应式图像的断点。需设置 `layout` 的值。通常不使用完整列表，而是根据源和输出大小进行过滤。使用的默认值取决于是使用本地还是远程图像服务。对于远程服务，使用更全面的列表，因为只生成所需的尺寸。对于本地服务，列表较短，以减少生成的图像数量。
